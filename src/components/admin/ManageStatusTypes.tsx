...
const ManageStatusTypes: React.FC = () => {
  ...
  const { register, handleSubmit, reset, formState: { errors } } = useForm<StatusTypeForm>()
  ...
  const fetchStatusTypes = async () => {
    ...
    const statusTypesData = statusTypesSnapshot.docs.map(doc => ({ id: doc.id, name: doc.data().name }))
  ...
  return (
    <div className="max-w-md mx-auto mt-8">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 mb-8">
        <div>
          <label htmlFor="name" className="block text-sm font-medium text-gray-700">New Status Type</label>
          <input
            type="text"
            id="name"
